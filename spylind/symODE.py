...
Plan: a class called ODESolver. Takes a symbollic dy_dt as input, as well as as python or symbollic functions for indep variables

It will/can:
    1. Do some preprocessing to simplify and eliminate unchanging state variables
    2. Will prepare a fast(ish) function that evaluates the derivative, and only evolves the necessary bits
...
def expand_list_to_re_im(sym_list):
    ...
    for sym in sym_list:
        if sym.is_real:
            ...
            re_im_out_L.append()
    complex_syms = [sym for sym in sym_list if not sym.is_real ]# all complex symbols


def expand_to_re_im(eqD, bNumericalRHS=False):#, indep_syms):
    """ Take an system of symbol: expression pairs and expand any complex symbols
    to their real/imaginary components. This means replacing the LHS symbols,
    so that the total number of equations will increase. It also means replacing
    any complex RHS symbols by their real and imaginary components. The independent
    RHS symbols are given in indep_syms (or it's generated within)

    Returns: real_eqD -> the realified map
            complex_subsD -> a mapping from complex symbols to real-imaginary components
    ...
    complex_subsD = {sym:val[0] + sm.I*val[1] for sym,val in symbol_mapD.items() if not sym.is_real }
    ...
    return dict( zip(new_lhsL, new_rhsL) ), complex_subsD, symbol_mapD# lhs_sym_mapD
...
def realify(f, which_complex_in=[], which_complex_out =[]):
    """ Take a function with a mixture of real and complex arguments and make
    it a 'real' function by expanding the complex arguments and complex outputs
    to real/imaginary components

    input: E.g. f(r_arg1, c_arg2, c_arg3, r_arg4)-> c_out1, r_out2
            which_complex_in = [0,1,1,0]
            which_complex_out= [1,0]
    Returns: E.g f_out(r_arg1, c_arg2_r, c_arg2_i, c_arg3_r, c_arg3_i, r_arg4)
                        -> c_out1_r, c_out1_i, r_out2


    NOTE: The current implentation is going to be super slow. In the longer run,
    this will have to be addressed. It should also probably use type hints instead
    of the current which_complex_in/out format.
    Major question: Can this be made to work with tensorflow??
    """
    #import inspect
    #real_sig = inspect.signature(f)
    N_orig_in = len(which_complex_in)#len(real_sig.parameters)
    N_orig_out = len(which_complex_out)
    #N_complex = sum(which_complex_in)
    #N_new_args = len(real_sig) - N_complex
    #N_real = len(real_sig) - N_complex #?)
    ...
    def wrap_func(*real_args):
        ...
        for k in range(N_orig_in):
            if which_complex_in[k]:
                fin_args.append(real_args[l] + 1j*real_args[l+1])
                l+=1
            else:
                fin_args.append(real_args[l])
            l+=1
...
class ODESolver(object):
    ...
    def __init__(self, dy_dtD, dims = {}, tSym = sm.symbols('t'), driving_syms=[],state_dep_syms=[], bDecompose_to_re_im=False, backend='numpy', default_dtype=np.complex128):
        ...
        self.tSym = tSym
        self.default_dtype = default_dtype#np.float64 if bDecompose_to_re_im else np.complex128

        dy_dtD = {sym:sm.sympify(ex) for sym,ex in dy_dtD.items()} #Take care of any python data types
        if bDecompose_to_re_im:
            propagated_state_syms, stationary_state_syms, indep_syms = self.sort_symbols(dy_dtD)
            ...
            #lhs_syms_realified = list(dy_dtD.keys())

        self.dy_dtD = dy_dtD
        self.sim_size = int(len(dy_dtD)*np.product(self.dim_shape))
        self.state_shape = tuple([len(dy_dtD)]+ [dim.size for dim in self.dimAxes] )
        ...
        state_func_sig = [self.tSym] + self.dimSyms + list(propagated_state_syms) + \
                list(stationary_state_syms) + driving_syms
        if bDecompose_to_re_im:
            ensure_list = lambda el: el if np.iterable(el) else [el]
            state_func_sig = [ensure_list(symbol_mapD[sym]) if sym in symbol_mapD else sym  for sym in state_func_sig]#expand_to_re_im(state_func_sig)
            state_func_sig = chain(*state_func_sig)
        self.state_func_sig = state_func_sig
        print(self.symsD)
        print("state dependent functions should have signature {}".format(self.state_func_sig ))

    @staticmethod
    def sort_symbols(eqD):
        """Take a dict defining symbols and their derivatives, and sort out:
            1. Which LHS symbols don't change
            2. Which RHS symbols need specifying
            3. Which LHS symbols actually need to be kept track of.
        """
        state_syms = list(eqD.keys())
        #eqD_1 = {lhs:rhs for lhs, rhs in eqD.items() if rhs!=0} #the non stationary symbols
    ...
    def set_initial_conditions(self,par0={}, bRealified=False):
        ...
        if isinstance(par0, Mapping):
            par0 = np.array( [par0[key] for key in self.symsD.prop_state_syms], dtype=self.default_dtype)
        ...
        #pdb.set_trace()
        if par0.ndim==1 and len(self.dimAxes)>0: # Assume a uniform initial state
           par0 = par0.reshape(-1, *len(self.dimAxes)*[1])
           newShape = (len(self.symsD['prop_state_syms']), *[np.size(ax) for ax in self.dimAxes]  )
    ...
    def set_driving(self, tDepFD={}):
        ...
        if not isinstance(tDepFD, Mapping): #Then we'll assume it's an iterable
            tDepFD = {sym: el for sym,el in zip(self.symsD.driving_syms, tDepFD)}
        subsD = {}
    ...
    def set_state_dep_funcs(self, stateDepFD={}):
        ...
        if self.bDecompose_to_re_im: #Split into real and imagiunary bits
            ...
            for sym, F in stateDepFD.items():
                F_flat = flatten_inputs(F, nDims, nStates, nDriving) #F_flat takes flattened complex inputs
                which_complex_out = [0 if sym.is_real else 1]
                F_ri_flat = realify(F_flat, which_complex_in, which_complex_out) #F_ri_flat takes flattened real inputs
                F_ri =  lambda t, dimAxes, states, driving: F_ri_flat(t, *dimAxes, *states, *driving)
    ...
    def setup(self, bDoSimplify=False, bForceStateDimensions=False):
        """Steps:
        ...
        ## DO SUBSTITUTIONS
        dy_dtD = self.dy_dtD
        dy_dtD = {sym:ex.subs(self.stateDepSubsD).subs(self.tDepSubsD) for sym,ex in self.dy_dtD.items()}
        if bDoSimplify:
    ...
    def make_d_dt_fast(self, eqD, t_dep_FD, state_dep_FD, bForceStateDimensions=False):
        ...
        state_syms = list(eqD.keys())
        rhs = list(eqD.values())
        input_syms = [self.tSym] + self.dimSyms +state_syms + t_dep_syms + state_dep_syms
        ...
        if self.backend=='numpy':
            ...
            if bForceStateDimensions:
                d_dtF0 = d_dtF
                d_dtF = lambda *args: [np.broadcast_to(out, self.dim_shape) for out in d_dtF0(*args) ]
                
            flatten_state = lambda state: state.reshape(-1)
            unflatten_state = lambda state, shape =self.state_shape: state.reshape(*shape)
            def dy_dt(t, cur_state_flat,
                    dimAxes =preShaped_dimAxes,
                    Npars = self.Npars,
                    state_shape = self.state_shape,
                    d_dt_prealloc = np.zeros( self.state_shape, dtype=self.default_dtype),
                    d_dtF = d_dtF,
                ...
                if t>10:
                    pass
                    #pdb.set_trace()
                state_dep_vals = [f(t, dimAxes, state, driving_vals) for f in state_dep_f]
                d_dt_prealloc[:] =  d_dtF(t, *dimAxes, *state, *driving_vals, *state_dep_vals)
            ...
            d_dtF = sm.lambdify( input_syms, rhs, modules=["tensorflow", {'conjugate':tf.math.conj} ])
            d_dtF = tf.function(d_dtF, experimental_compile=True, experimental_relax_shapes=True)
            flatten_state = lambda state: tf.reshape(state, [-1])#.reshape(-1)
            unflatten_state = lambda state, shape =self.state_shape: tf.reshape(state, shape)
            ...
            def dy_dt(t, cur_state_flat, dimAxes =preShaped_dimAxes , Npars = self.Npars,
                    state_shape = tuple(self.state_shape),
                    d_dtF = d_dtF, flatten=flatten_state, unflatten=unflatten_state,
                    state_dep_f = tuple(state_dep_f), t_dep_f0= tf.function(t_dep_f[0], experimental_compile=True), sim_size=self.sim_size , astate=np.ones(self.dim_shape, dtype='c16')):
                #print(dimAxes[0].shape)
                #state = unflatten(cur_state_flat)
                state = tf.reshape(cur_state_flat, state_shape)
                #driving_vals = [f(t) for f in t_dep_f]
                #driving_vals = [t_dep_f[0](t)]
                #state_dep_vals = [f(t, dimAxes, state, driving_vals) for f in state_dep_f]
                #state_dep_vals = []
                #d_dt_prealloc =  d_dtF(t, *tf.unstack(dimAxes), *tf.unstack(state), t_dep_f[0](t))#*driving_vals, *state_dep_vals)
                #E = tf.constant(t_dep_f0(t), dtype=tf.float64)
                E = tf.cast(t_dep_f0(t), dtype=tf.complex128)

                #print(type(E), E.shape)
                d_dt_prealloc =  d_dtF(tf.cast(t, dtype=tf.complex128), *tf.unstack(dimAxes), *tf.unstack(state), E)#*driving_vals, *state_dep_vals)
                #d_dt_prealloc =  d_dtF(t, dimAxes[0], state[0], state[1], state[2], E)#*driving_vals, *state_dep_vals)
                #d_dt_prealloc =  d_dtF(7.0, linspace(-1,1,1000)+0j, *state, 7+1j)#*driving_vals, *state_dep_vals)
                #call_d_dtF(t, dimAxes, state,driving_vals, state_dep_vals)
    ...
    def integrate(self, tSteps, max_step_size=.1, atol=1e-12, rtol=1e-6):
        if self.backend == 'numpy':
            ...
            if self.bDecompose_to_re_im:
                r.set_integrator('vode', method='adams',max_step=max_step_size, atol=atol, rtol=rtol)#,order=5) # or adams
            else:
                r.set_integrator('zvode', method='adams',max_step=max_step_size, atol=atol, rtol=rtol)#,order=5) # or adams
            ...
            for k,tNext in enumerate(self.tSteps):
                ...
                self._online_process_func(cur) #probably reshaping, calculating fields etc
                #print('.',end='', flush=True)
            return np.array(self.outputL)
        elif self.backend=='tensorflow':
            y_init = tf.constant(self.flatten_state(self.par0), dtype=tf.complex128)
...
# FOR REFERENCE ONLY
def get_dydt_from_dict(tSym, eqD, indepSubD={}, bIncludeT=True, modules="numpy", bSimplifyRHS=False): #Is t an independent symbol? Only if it appears explicitly I guess
    ...
    if 1:
        indepFuncD= {sym:indepSubD.pop(sym) for sym,expr in list(indepSubD.items()) if callable(expr)  }
        subsD = {sym:sm.sympify(indepSubD.pop(sym)) for sym,expr in list(indepSubD.items())  }
    ...
    eqD_1 = eqD#{key:val for key,val in list(eqD.items()) if val!=0 } #Non-stationary equations
    ...
    extIndepSyms = [sym for sym in freeSyms0 if sym not in stateSyms and sym not in intIndepSyms and sym!=tSym]
    ...
    d_dtF = sm.lambdify(tuple(lamInputSyms), tuple(rhs), modules=modules) # could also split into real and ind imaginary parts
    ...
    def dy_dt(t, curState, indepInputs=[]):
        intIndepVals = curIndepVarsF(t) # the current value of t might be included in here?
...
if __name__=="__main__":
    ...
    #
    import helper as h
    tS, zS, DeltaS, E_intS= sm.symbols('t, z, Delta, E_int')
    H0, (Esym,H1), ((DeltaSym,H_delt),), c_opL = h.constructHamiltonian(gsEnergies= [0], esEnergies=[0])
    Htot = H0 + H1 + DeltaSym*H_delt
    # MAKE dy_dtD
    (diagL,diag_dtL), (cohL, coh_dtL), e_op_L = h.makeMESymb(Htot, c_opL, [H1])
    polS = e_op_L[0].subs(Esym,1) # polarisation due to atoms
    #
    dy_dtD =  dict(zip(diagL+ cohL, diag_dtL + coh_dtL) )

    def EintF(*args):
        t,z,delta, *state, Ef = args
        return exp(1j*z*2*pi)*Esym
    EintF = makeEinternalF(polS, dims, line_shape, cavity_params={})

    ## Make state_dep_funcs (i.e. calcExcitedModes)
    ode_s = ODESolver(dy_dtD,  dims={zS:linspace(0,1,5), DeltaS:linspace(-1,1,3)}, driving_syms=[Esym])
    ode_s.set_state_dep_funcs({E_intS: EintF})

    # Constant driving
    ode_s.set_driving({Esym: lambda t: 0.1})

    # Set initial conditions
    par0 = {key:np.zeros((5,3)) for key in diagL + cohL}
    par0[diagL[0]] += 1
    ode_s.set_initial_conditions(par0)

    #Actually do things
    ode_s.setup()
    ode_s.integrate(np.linspace(0,1,100))

    # Plot results
    # ode_s.d_dt(0, *par0, EintF(0))
    # PLAN: Need to get an actual simulation running
    # e.g.


    if 0:
        p1S,p2S,zS, tS, F, F2, E_intS, DeltaS= sm.symbols('p_1, p_2, z, t, F, F2, G Delta')

        d_dtD = {p1S:p1S**2,
                p2S:zS-2+ F2 + E_intS }

        ode_s = ODESolver(d_dtD,  dims={zS:linspace(0,1,5), DeltaS:linspace(-1,1,3)}, driving_syms=[F, F2])
        ode_s.set_driving({F:sm.cos(10*tS), F2:lambda t:t**2})
        ode_s.set_state_dep_funcs({E_intS:lambda t,p1,p2, F, F2:F*DeltaS*(p1**2 + p2)})
        ode_s.set_initial_conditions({p1S:arange(15).reshape(5,3),
                p2S:arange(15).reshape(5,3)})
        ode_s.setup()
        #import helper
        #H0, (Esym,H1), Hlst, c_opL = h.constructHamiltonian([-1,1], esEnergies=[0])


    def test_ODESolver():
        p1S,p2S,zS, tS, F, F2, G, DeltaS= sm.symbols('x, y, z, t, F, F2, G Delta')
